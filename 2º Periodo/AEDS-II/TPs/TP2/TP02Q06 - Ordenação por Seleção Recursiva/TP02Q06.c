...
void selecao(char Titulos[][100], char Ids[][100], int numTitulos) {
    char tempTitulo[100];
    char tempId[100];

    for (int i = 0; i < numTitulos - 1; i++) {
        int menor = i;
        for (int j = i + 1; j < numTitulos; j++) {
            if (strcmp(Titulos[menor], Titulos[j]) > 0) {
                menor = j;
            }
        }
        if (menor != i) {
            ...
            strcpy(Titulos[i], Titulos[menor]);
            strcpy(Titulos[menor], tempTitulo);

            // Trocar ID correspondente também
            strcpy(tempId, Ids[i]);
            strcpy(Ids[i], Ids[menor]);
            strcpy(Ids[menor], tempId);
        }
    }
}
...
int main()
{
    ...
    selecao(Titulos, Ids, numTitulos);
