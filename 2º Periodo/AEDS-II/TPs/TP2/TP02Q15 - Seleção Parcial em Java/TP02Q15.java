...
public class TP02Q15 {
    ...
    public static void Selecao(ArrayList<String> Titulos , int k){
       
       
       int n = Titulos.size(); // pega o tamanho da lista 

        for (int i = 0; i <k; i++) {
            int menor = i;

            // Encontrar o menor elemento do restante da lista
            for (int j = i + 1; j < n; j++) {
                if (Titulos.get(j).compareTo(Titulos.get(menor)) < 0) {
                    menor = j;
                }
            }

            // Trocar o elemento atual com o menor encontrado
            Collections.swap(Titulos, i, menor);
        }
    ...
    public static void main(String[] args) throws IOException {
       ...
       ArrayList<Show> todosShows = new ArrayList<>();
        ...
        while ((CSV = br.readLine()) != null) {
            ...
            todosShows.add(s);
            Titulos.add(s.getTitle());
           
            mapaShows.put(s.getTitle(), s);
