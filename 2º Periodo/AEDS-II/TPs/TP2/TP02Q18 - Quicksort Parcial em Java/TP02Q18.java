...
public class TP02Q18 {
    ...
    static SimpleDateFormat sdf = new SimpleDateFormat("MMMM d, yyyy", Locale.ENGLISH);

    public static void quicksort(ArrayList<Show> lista, int esq, int dir) throws ParseException {
        int i = esq;
        int j = dir;
        Show pivo = lista.get((esq + dir) / 2);

        while (i <= j) {
            while (compare(lista.get(i), pivo) < 0) i++;
            while (compare(lista.get(j), pivo) > 0) j--;
    ...
    // Método de comparação com critério de desempate
    public static int compare(Show a, Show b) throws ParseException {
        Date dateA = parseDate(a.getDateAdded());
        Date dateB = parseDate(b.getDateAdded());
    ...
    // Método seguro para converter string em Date
    private static Date parseDate(String dateStr) {
        try {
            if (dateStr == null || dateStr.equals("NaN") || dateStr.trim().isEmpty()) return null;
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        while ((CSV = br.readLine()) != null) {
            Show s = new Show();
            s.ler(CSV);
            if (lista.contains(s.getShowId()) && !s.getDateAdded().equals("NaN")) {
                shows.add(s);
        ...
        // Ordena os shows por data com QuickSort
        quicksort(shows, 0, shows.size() - 1);
