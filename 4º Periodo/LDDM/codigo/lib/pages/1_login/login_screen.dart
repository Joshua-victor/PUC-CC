// lib/pages/1_login/login_screen.dart

import 'package:codigo/pages/2_signup/signup_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final dbService = DatabaseService(); // Seu serviço de banco local
  ...
  Future<void> _handleLogin() async {
    ...
    if (loginInput.isEmpty || senha.isEmpty) {
      _showError('Por favor, preencha login e senha.');
      return;
    }

    bool loggedViaFirebase = false;

    try {
      // ---------------------------------------------------------
      // 1. TENTATIVA VIA FIREBASE (PRIORIDADE MÁXIMA)
      // ---------------------------------------------------------
      ...
      if (isEmail) {
        try {
          ...
          if (userCredential.user != null) {
            print("✅ SUCESSO: Logado no Firebase Auth!");
            loggedViaFirebase = true;

            if (mounted) {
              // --- PULO DO GATO ---
              // Como logamos no Firebase, NÃO vamos tentar ler o SQLite (que está dando erro).
              // Passamos userId: 1 apenas para a Home não quebrar (já que ela espera um int).
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (context) => const MainScaffold(userId: 1)),
                    (Route<dynamic> route) => false,
              );
              return; // <--- IMPORTANTE: Encerra aqui para não rodar o código do SQLite abaixo
            }
          }
        } catch (firebaseError) {
          print("⚠️ Aviso: Login Firebase falhou (senha errada ou sem internet). Motivo: $firebaseError");
          // Se falhar no Firebase, deixamos o código descer para tentar o banco local (caso exista)
        }
      }

      // ---------------------------------------------------------
      // 2. TENTATIVA VIA BANCO LOCAL (FALLBACK)
      // ---------------------------------------------------------
      // Só chega aqui se o Firebase falhou ou não foi tentado
      if (!loggedViaFirebase) {
        print("🔄 Tentando autenticação via Banco Local (SQLite)...");

        try {
          // Tenta buscar no SQLite
          final userMap = await dbService.getUserByCredentials(loginInput, senha);

          if (userMap != null && mounted) {
            final int userId = userMap['id_usuario'];
            print("✅ SUCESSO: Logado via SQLite Local.");

            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) => MainScaffold(userId: userId)),
                  (Route<dynamic> route) => false,
            );
          } else {
          ...
          // AQUI CAPTURAMOS O ERRO "Database file doesn't exist"
          print("❌ ERRO NO SQLITE: O banco de dados local não foi encontrado ou está corrompido.");
          print("Detalhe do erro: $sqliteError");

          // Mostra mensagem amigável sem travar o app
          _showError('Erro: Banco de dados local não encontrado. Tente reinstalar o app ou verifique a internet para login online.');
        }
      }

    } catch (e) {
      print("Erro geral não tratado: $e");
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              // Botões sociais (apenas visuais por enquanto)
              _buildSocialButton(
  ...
  // --- WIDGETS AUXILIARES (Mantidos iguais ao original) ---
